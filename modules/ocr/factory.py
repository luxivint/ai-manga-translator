...
class OCRFactory:
    ...
    def create_engine(
        ...
        # Always use local/direct-provider OCR engines (no hosted/managed API path).
    ...
    def _create_new_engine(
        ...
        general = {
            ...
            'GPT-5.4-mini': lambda s: cls._create_gpt_ocr(s, ocr_model),
            'Gemini-2.0-Flash': lambda s: cls._create_gemini_ocr(s, ocr_model),
            'Qwen3-VL-Flash Grid OCR': lambda s: cls._create_qwen_grid_ocr(s, 'qwen3-vl-flash'),
            'Qwen3-VL-Plus Grid OCR': lambda s: cls._create_qwen_grid_ocr(s, 'qwen3-vl-plus'),
            'Qwen3.6-Flash Grid OCR': lambda s: cls._create_qwen_grid_ocr(s, 'qwen3.6-flash'),
            'Qwen3.6-Plus Grid OCR': lambda s: cls._create_qwen_grid_ocr(s, 'qwen3.6-plus'),
        }
    ...
    @staticmethod
    def _create_qwen_grid_ocr(settings, model: str) -> OCREngine:
        credentials = settings.get_credentials(settings.ui.tr("Qwen"))
        engine = QwenGridOCR()
        engine.initialize(api_key=credentials.get("api_key", ""), model=model)
        return engine
