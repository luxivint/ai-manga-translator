...
from .rtdetr_v4_onnx import RTDetrV4SInt8ONNXDetection
...
class DetectionEngineFactory:
    ...
    def create_engine(
        ...
        engine_factories = {
            ...
            'RT-DETR-v4-s-int8': cls._create_rtdetr_v4s_int8,
        }
    ...
    @staticmethod
    def _create_rtdetr_v4s_int8(settings, backend: str = 'onnx'):
        """Create and initialize the RT-DETR v4-s int8 (ONNX-only) detection engine."""
        device = resolve_device(settings.is_gpu_enabled(), 'onnx')
        engine = RTDetrV4SInt8ONNXDetection(settings)
        engine.initialize(device=device)
        return engine
