...
def generate_mask(img: np.ndarray, blk_list: list[TextBlock], default_padding: int = 5) -> np.ndarray:
    ...
    for blk in blk_list:
        ...
        # 9) Dilate the block mask. Speech-bubble text needs a tighter mask:
        # over-dilation reaches bubble outlines and makes the inpainter erase borders.
        dil_kernel = np.ones((kernel_size, kernel_size), np.uint8)
        iterations = 4
        if getattr(blk, 'text_class', None) == 'text_bubble':
            iterations = int(getattr(blk, 'bubble_mask_dilate_iterations', 0) or 2)
        dilated = imk.dilate(block_mask, dil_kernel, iterations=iterations)
