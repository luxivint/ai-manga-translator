        ...
        if model_md5:
            ...
            if model_md5 == _md5:
                ...
                try:
                    ...
                    logger.error(
                        f"Model md5: {_md5}, expected md5: {model_md5}, wrong model deleted. Please re-run the pipeline to re-download it."
                        f"If you still have errors, please try download model manually first.\n"
                    )
                except:
                    logger.error(
                        f"Model md5: {_md5}, expected md5: {model_md5}, please delete {cached_file} and re-run the pipeline."
                    )
                    raise RuntimeError(
...
def handle_error(model_path, model_md5, e):
    ...
    if _md5 != model_md5:
        try:
            ...
            msg = f"Model md5: {_md5}, expected md5: {model_md5}, wrong model deleted. Please re-run the pipeline to re-download it."
            logger.error(msg)
            raise RuntimeError(msg)
        except Exception:
            msg = f"Model md5: {_md5}, expected md5: {model_md5}, please delete {model_path} and re-run the pipeline."
            logger.error(msg)
            raise RuntimeError(msg)
    else:
